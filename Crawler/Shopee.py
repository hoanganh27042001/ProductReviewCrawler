...
class Shopee:
    ...
    def search_product_by_keyword(self, keyword: str, limit: str = '20'):
        ...
        products = []
        url = r'https://shopee.vn/api/v4/search/search_items?by=relevancy&keyword={}&limit=10&newest=0&order=desc&page_type=search&scenario=PAGE_GLOBAL_SEARCH&version=2'.format(keyword)
        print('url = ', url)
        # Shopee API request
    ...
    def find_reviews_by_keyword(self, keyword: str, limit: int = 20):
        ...
        for product in product_lists:
            ...
            collections.append({'item_id': itemid, 'name': name, 'shop_id': shopid, 'reviews': review, 'source': 'shoppee'})
        return collections
